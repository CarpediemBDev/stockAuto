...
StockAuto의 AI 작업 규칙과 스킬 원본은 프로젝트 내부에서 관리합니다.

- **`AGENTS.md`**: StockAuto 최상위 AI/개발 작업 규칙
- **`docs/AI_WORKFLOW.md`**: 작업 등록, 변경 영향 기록, 검증, 인수인계 절차
- **`skills/common-ai-workflow`**: 새 프로젝트에도 재사용 가능한 공통 AI 작업 원칙
- **`skills/stockauto-release-risk-auditor`**: StockAuto 전용 릴리즈/계산/프로세스 리스크 감사 지침
- **`scripts/verify_harness.py`**: 프로젝트 검증 하네스

AI 에이전트는 사용자 폴더의 `.codex/skills`를 원본으로 보지 않고, 이 저장소의 `skills/` 아래 파일을 기준으로 작업합니다.
