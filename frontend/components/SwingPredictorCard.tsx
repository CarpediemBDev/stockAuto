...
import { Compass, ShieldCheck, Flame, Layers, TrendingUp, TrendingDown, HelpCircle, Activity } from 'lucide-react';
...
export function SwingPredictorCard() {
  ...
  // 일봉 스윙 분석은 연산량이 크므로 45초 주기로 여유롭게 폴링 수행
  usePolling(fetchSwingCandidates, 45000);
  ...
  return (
    <div className="bg-zinc-950 border border-zinc-800 rounded-3xl p-6 shadow-2xl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 pb-4 border-b border-zinc-900 gap-4">
        ...
        <span className="text-[10px] bg-zinc-900 text-zinc-400 border border-zinc-800 font-mono px-3 py-1 rounded-full flex items-center gap-1.5 self-end md:self-auto select-none">
      ...
      {candidates.length === 0 ? (
        <div className="py-16 text-center bg-zinc-900/20 rounded-2xl border border-dashed border-zinc-800">
          ...
          <p className="text-sm font-bold text-zinc-500">현재 완벽하게 수렴하여 매집 임계치에 도달한 스윙 종목이 없습니다.</p>
          <p className="text-xs text-zinc-600 mt-1">시장 상황에 상관없이 기준에 부합하지 않는 잡주는 절대 추천하지 않습니다.</p>
        </div>
      ) : (
