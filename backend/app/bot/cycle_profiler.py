...
class CycleProfile:
    ...
    def has_user_flows(self) -> bool:
        """사용자 흐름이 한 번이라도 시작됐는가.

        가동 유저가 없거나, 휴장이면서 보유 유저도 없어 루프가 조기 반환한 사이클은 요약할
        구간이 없으므로 줄을 남기지 않는다.

        ⚠️ 이 판정은 휴장 사이클을 걸러 주지 않는다. 휴장이어도 보유 종목이 있는 유저가
        한 명이라도 있으면 사용자 흐름이 시작되어 참이 된다(매도·매수 판정 단계에서만 빠진다).
        휴장 표본을 거르는 책임은 요약 줄의 session 표기를 읽는 쪽에 있다. 휴장 표본이
        기준선을 오염시켜 회귀를 가린 전례(2026-09)가 있다.
        """
        with self._lock:
