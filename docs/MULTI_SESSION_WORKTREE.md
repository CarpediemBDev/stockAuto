...
> **적용 대상: 이 저장소에서 파일을 수정하는 모든 주체** — Claude 세션 여러 개, 다른 AI 에이전트, 사람(IDE/터미널).
> 상위 규칙은 [`AGENTS.md`](../AGENTS.md) §9-1이며, 이 문서는 그 실행 세부를 담는다.
...
*   **두 번째 쓰기 세션이 필요해지면 그때 `git worktree add`로 전용 폴더를 만들고, 이 표와 당일 현황판에 배치를 기록한다.** 쓰지 않는 worktree는 `git worktree remove`로 정리해 표를 실제 상태와 일치시킨다.
