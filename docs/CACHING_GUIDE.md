...
> [!NOTE]
> **StockAuto 봇의 캐시 및 동시성 제어 전략 (Redis 도입)**
> 기존에는 서버 1대 기준 파이썬 전역 딕셔너리(`dict`)를 활용한 메모리 로컬 캐싱만 사용했습니다.
> 하지만 시스템이 고도화되고 멀티테넌시(Multi-Tenancy) 환경 및 사용자별 동시 주문 제어(수동 강제 청산과 봇 자동주문 충돌 방지 등)가 필요해지면서, **Redis를 공식 인프라로 도입**하였습니다.
> 현재 외부 API(야후 파이낸스) IP 차단 방지 등 단순 조회용 데이터는 로컬 TTL 캐시를 일부 병행하지만, **주요 상태 공유 및 분산 락(Distributed Lock) 등 핵심 주문 제어 영역에는 Redis를 적극 활용**하고 있습니다.
...
**결론:** 서버가 2대 이상이거나, 단일 서버라도 확실한 동시성 제어(Race Condition 방지)가 필요할 때는 WAS 내부에 캐시를 두지 않고, 공용으로 사용할 수 있는 전용 상태/캐시 서버(Redis)를 도입해야 합니다. StockAuto 프로젝트 역시 이러한 한계를 극복하고 안전한 트레이딩 동시성 제어를 위해 **Redis를 핵심 인프라로 도입 완료**했습니다.
