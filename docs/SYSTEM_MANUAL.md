...
### ⏱️ 매매 사이클 주기와 계측 로그 읽는 법

매매 루프(`main_trade_job`)는 1분 주기로 등록되어 있지만 **실제 간격은 사이클 소요 시간에 따라 늘어난다.** APScheduler interval 트리거는 다음 시각을 직전 *예정* 시각에서 뽑고 `max_instances=1`이라, 사이클이 60초를 넘기면 그 사이 도착한 틱은 뒤로 밀리지 않고 버려진다. 사이클이 61초 걸리면 다음 실행은 120초 뒤다(재현: `backend/tests/test_scheduler_cycle_interval.py`). 따라서 **사이클 수를 시간 단위로 쓰는 게이트는 만들지 않는다.** 방어·수확·손절 게이트는 모두 벽시계 시간을 요구하며, 방어 조치는 시간이 주도하고 횟수는 하한만 준다.

`backend/logs/stockauto.log`에 사이클마다 세 줄이 남는다.

| 로그 | 뜻 |
| :--- | :--- |
| `[Cycle] Interval since previous start` | 직전 사이클 시작과의 간격. 90초 이상이면 WARNING |
| `[Cycle] Duration: T s (session=…)` | 이번 사이클 총 소요 시간. 60초 이상이면 다음 틱이 버려진다. 세션을 판정하기 전에 끝난 사이클은 `session` 표기가 없다 |
| `[Cycle] Profile (session=…, N users, T s): …` | 구간별 누적 시간. 사용자 흐름이 한 번이라도 시작된 사이클에 남는다 — **휴장 사이클에도 남는다**(아래 참고) |

`Profile` 줄은 `구간=누적초/호출횟수(max 최대초)`를 누적 시간 큰 순서로 나열한다. 읽는 법:

- **구간은 중첩된다.** `user.exit_signals` 안에 `exit.analyze_single_ticker`·`db.micro_session.*`이 들어 있으므로 값을 서로 더하지 않는다.
- **`loop.blocked`가 사이클 시간의 대부분이면** 사용자 흐름이 병렬로 돌지 못하고 있다는 뜻이다. 사용자별 흐름은 `asyncio.gather`로 한 이벤트 루프에서 도는데, 동기 DB·브로커·pandas 작업이 루프를 붙잡으면 순차 실행으로 무너진다. 해법은 캐시·중복 제거가 아니라 동기 작업을 루프 밖으로 빼는 것이다.
- **`cycle.user_flows_wall`과 `user.*` 합을 비교한다.** 합이 벽시계를 크게 넘으면 병렬이 살아 있고, 비슷하면 무너진 것이다.
- **`exit.analyze_single_ticker` 호출 횟수**는 시그널 캐시에 없는 보유 종목을 사용자마다 따로 조회한 횟수다. 같은 티커를 여러 계정이 들고 있어도 중복 제거되지 않는다.

**기준선을 잴 때는 휴장 표본을 반드시 뺀다.** 휴장이면 매도·매수 판정을 건너뛰어 사이클이 짧게 끝나므로, 섞으면 회귀가 가려진다.

**`Profile` 줄이 남았다고 정규장 사이클인 것은 아니다.** 휴장이어도 보유 종목이 있는 유저가 한 명이라도 있으면 사용자 흐름이 시작되어(잠금·보유 동기화까지 돈다) 줄이 남는다. 루프가 조기 반환해 줄이 생략되는 것은 "가동 유저 없음" 또는 "휴장이면서 보유 유저도 없음"뿐이다. 반드시 `session=REGULAR_MARKET`인 줄만 골라 비교한다. 세션 값은 사이클 **시작** 시점 기준이며(`PRE_MARKET`·`REGULAR_MARKET`·`AFTER_HOURS`·`CLOSED`, 주말·NYSE 휴장일은 `CLOSED`), 개장·폐장 경계를 걸친 사이클은 시작 세션으로 표기된다. 재기동 직후 첫 사이클은 워밍업이라 세션과 무관하게 비교에서 뺀다. 2026-09 회귀(NY 정규장 간격 중앙값 8월 129~155초 → W37 426초)는 휴장 표본이 섞인 124초 기준선 때문에 한동안 발견되지 않았다. 비교는 NY 정규장끼리, 같은 주차 단위로 한다.
...
* **`TossBroker` (토스 연동용)**: 토스증권 API 클라이언트는 공식 OpenAPI 1.1.5의 `/api/v1/holdings`, `/api/v1/buying-power`, `/api/v1/orders` 계약에 맞춰 보존합니다. 개인 설정 화면에서 토스 인증키를 암호화 저장할 수 있지만, 현재 StockAuto에서는 검증 완료 전까지 토스 거래 실행 모드를 `SIMULATED`로만 제한합니다.
...
2. `broker_factory.py` 파일 상단의 `BROKER_REGISTRY` 딕셔너리에 신규 브로커의 사용자 노출 정보, 인증 클라이언트, 실제 지원 가능한 거래 모드별 실행 클래스를 매핑합니다.
   ...
   ```
3. 개인 투자 설정 화면은 `get_broker_catalog()`의 `supported_modes`를 기준으로 미지원 실행 조합을 비활성화합니다. 다만 토스처럼 실행 보류 중인 브로커도 `KEY_ONLY` 모드로 인증키를 먼저 암호화 저장할 수 있습니다. 저장된 키는 `verification_status=stored`로 표시되며, MOCK/REAL 자동매매 전환에는 사용할 수 없습니다.
4. 백엔드 설정 저장 API와 브로커 팩토리는 실제 거래 모드 전환 시 같은 capability 검사를 수행합니다. UI 우회 요청으로 토스 MOCK/REAL 전환을 시도해도 실행 단계 전에 차단됩니다.
...
4. **🔒 Redis 주문 동시성 경계와 영구 주문 원장**

   - `UserSettings.is_running`은 사용자가 선택한 시작·정지 상태만 저장합니다. 주문 접수, 미체결, 재조정 또는 강제 청산은 이 값을 자동 변경하지 않습니다.
   - Redis 사용자 작업 락은 같은 사용자의 자동매매 주기와 수동 강제 청산이 동시에 실행되지 않게 합니다. 종목 주문 락은 같은 사용자·같은 티커의 중복 제출을 차단합니다.
   - Redis 락을 획득한 뒤에만 `BrokerOrder` 주문 의도를 생성합니다. Redis 장애 시 주문은 fail-closed되지만 조회·인증·대시보드 API는 계속 동작합니다.
   - Redis는 일시적인 실행 경계를 담당하고, `broker_orders` DB 원장은 미해결 주문을 다음 주기와 서버 재시작 이후까지 차단합니다.
...
주문 기능을 사용하기 전에 백엔드 폴더에서 Redis를 기동하고 `.env.<profile>`의 `REDIS_URL`을 확인합니다.

```bash
docker compose up -d redis
```

로컬 Compose는 Redis 포트를 `127.0.0.1:6379`에만 바인딩합니다. 운영 환경은 관리형 Redis의 내부 주소를 `REDIS_URL`로 주입합니다.
