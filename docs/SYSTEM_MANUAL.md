...
| `[Cycle] Duration` | 이번 사이클 총 소요 시간. 60초 이상이면 다음 틱이 버려진다 |
| `[Cycle] Profile (N users, T s): …` | 구간별 누적 시간. 매매 흐름이 실제로 돈 사이클에만 남는다 |
...
**기준선을 잴 때는 휴장 표본을 반드시 뺀다.** 휴장이면 사용자 흐름을 건너뛰어 사이클이 짧게 끝나므로, 섞으면 회귀가 가려진다. 2026-09 회귀(NY 정규장 간격 중앙값 8월 129~155초 → W37 426초)는 휴장 표본이 섞인 124초 기준선 때문에 한동안 발견되지 않았다. 비교는 NY 정규장끼리, 같은 주차 단위로 한다.
