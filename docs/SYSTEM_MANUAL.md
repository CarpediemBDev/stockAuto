...
* **`KISBroker` (한투 연동용)**: 한투 API 실전 및 모의투자 API```mermaid
...
sequenceDiagram
    ...
    participant GlobalDaemon as 글로벌 폴링 스레드 (Global Daemon)
    ...
    Note over GlobalDaemon, TG: [서버 가동] 단 하나의 글로벌 폴링 데몬 가동 (settings.TELEGRAM_BOT_TOKEN)

    

    loop 실시간 메시지 모니터링 (Long-Polling)

        GlobalDaemon->>TG: getUpdates 요청

        TG-->>GlobalDaemon: 사용자 메시지 수신 (msg_chat_id 포함)

    end

    

    alt 1. 미연동 가입자의 딥링크 연동 (/start <username>)

        GlobalDaemon->>Engine: _process_global_message(msg_chat_id, text)

        Engine->>DB: username 조회 ➔ 해당 User Settings에 telegram_chat_id 자동 등록

        Engine->>TG: sendMessage (연동 완료 축하 메시지)

        TG-->>User: "연동이 완료되었습니다!" 메시지 수신

    else 2. 기연동 가입자의 명령어 처리 (/status, /run, /stop)

        GlobalDaemon->>Engine: _process_global_message(msg_chat_id, text)

        Engine->>DB: chat_id 기반의 유저 조회 ➔ _process_command(user_id, text)

        Engine->>TG: sendMessage (포트폴리오, 상태 혹은 가동 메시지 마크다운 전송)

        TG-->>User: 결과 메시지 수신

    end

```상 성격의 주황색 테두리와 함께 **`KIS MOCK`** 배지 표시.
