...
## 🌟 [Phase 31] 수수료 절감을 위한 진입 장벽 상향 및 가격 이탈 2회 연속 확정 버퍼 구축 — 완료 [x]

- [x] **[상수 및 컷오프 상향]** `scheduler.py` 내의 자동매수 진입 컷오프 점수를 상승장 `85점`, 하락장 `95점`으로 상향하고 `scanner.py` 및 `backtest_engine.py`와 완벽 동기화
- [x] **[피라미딩 조건 강화]** `scheduler.py` 및 `backtest_engine.py` 내의 1:2:6 피라미딩(불타기) 추가 매수 수익률 임계치를 2단계 `+3.0%`, 3단계 `+6.0%`로 대폭 상향 조정
- [x] **[2회 연속 이탈 확정 버퍼]** `scheduler.py` 내에 인메모리 이탈 추적 캐시(`BREACH_COUNT_CACHE`)를 탑재하여 손절선 및 트레일링 스탑 가격 이탈이 2회 연속(2분 연속 종가) 감지될 때만 매도하도록 구현 및 `backtest_engine.py`에 동일 로직 이식
- [x] **[모의투자 가상 잔고 자동 충전 버그 핫픽스]** 전량 청산 시 실현 성과와 무관하게 1,000만 원으로 복구되던 버그를 `TradeLog` 누적 역산 방식으로 수정 완료하여 손익이 영구 축적 보존되도록 개선
- [x] **[문서 및 가이드 최신화]** `docs/TRADING_STRATEGY.md` 및 `docs/FILTER.md` 내에 진입 컷오프 상향, 피라미딩 강화 및 2회 연속 이탈 확정 버퍼 관련 내용 반영 및 현행화
